<script>
	...
	//default is the home hero content - the case with route example /services is for services hero page content
...
with the world.`
			...
			case '/about':
				return {
					title: 'About Us',
					description: 'Learn more about our mission to transform communications.'
				};
...
>
	<!-- Hero -->
	<div class="relative {route !== '/' ? 'h-150' : ' h-175 md:h-screen'}  overflow-hidden">
		{#key route}
			<header
				class="absolute bottom-0 left-0 md:w-[650px] z-99 px-6 md:px-16 py-10 md:py-16"
				in:fade={{ duration: 750 }}
			>
				<h1 class="text-white text-4xl font-bold {route !== '/' ? 'lg:text-7xl' : 'lg:text-7xl'}">
					{heroContent.title}
				</h1>
