<script>
	...
	const handleSubmit = async (event) => {
		...
		const response = await fetch('/api/send-email', {
			...
			body: JSON.stringify({ name, email, message, companyName, industry, phoneNumber, interest })
		});
	...
	>
		<div class="bg-[#0a0e30] md:col-span-4 p-10 text-white">
			...
			<p class="mt-4 leading-7 text-gray-200">
				If there’s anything we can assist you with, whether it’s products or services, don’t
				hesitate to reach out! We’re here to connect you with the best path to the future. Stay
				connected using the details below. Thank you!
			</p>
			...
			<div class="grid grid-cols-[max-content_1fr] items-center mt-5 gap-3">
				...
				>
					<path
						...
						stroke-width="1.5"
					/>
					<path d="M15 5H9" stroke="#fff" stroke-width="1.5" stroke-linecap="round" />
		...
		{#if thankyou}
			<div class="md:col-span-8 p-10 text-center min-h-[655px]">
				...
				<p class="text-gray-700">
					{firstName} Your message has been sent successfully. <br /> We will contact you shortly
				</p>
			</div>
		{:else}
